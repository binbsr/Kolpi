...
public class KolpiDbContext(DbContextOptions options) : DbContext(options)
{
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        if (!optionsBuilder.IsConfigured)
        {
            optionsBuilder.UseSqlServer("Server=(localdb)\\mssqllocaldb;Database=KolpiDb;Trusted_Connection=True;MultipleActiveResultSets=true");
        }
    }

    // Kolpi Db tables
    public DbSet<Question> Questions { get; set; } = default!;
    public DbSet<QuestionTag> QuestionTags { get; set; } = default!;
